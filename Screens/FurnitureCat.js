...
const FurnitureCat = () => {
  ...
  useEffect(() => {
    ...
    const furnitureItemsRef = ref(db, 'FurnitureItems');

    const onSnapshot = onValue(furnitureItemsRef, (snapshot) => {
      ...
      if (items) {
        ...
        setFurnitureItems(itemList);
      }
    });

    return () => off(furnitureItemsRef, onSnapshot);
  }, []);
